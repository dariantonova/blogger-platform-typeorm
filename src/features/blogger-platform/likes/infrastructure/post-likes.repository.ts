...
export class PostLikesRepository {
  ...
  async save(like: PostLike): Promise<PostLike> {
    if (!like.id) {
  ...
  async findByUserAndPost(
    ...
    return findResult[0] ? PostLike.reconstitute(findResult[0]) : null;
  }
  ...
  private async createPostLike(like: PostLike): Promise<void> {
    const createQuery = `
